...
class TracedEnv:
    ...
    def extract_game_state(self, objects, info):
        ...
        OCAtari Q*bert categories: Player, Cube, Disk, PurpleBall, RedBall,
        GreenBall, Coily, Sam (plus None for absent objects).

        Returns a dict with:
          - "Player": Q*bert state {x, y, w, h, dx, dy}
          - "Enemies": list of enemy states [{x, y, w, h, dx, dy, type}, ...]
            (Coily snake, red/purple balls)
          - "Friendlies": list of friendly entity states [{x, y, w, h, dx, dy, type}, ...]
            (Sam — catch for 300 pts, GreenBall — catch for 100 pts)
          - "Disks": list of flying disk states [{x, y, w, h}, ...]
        ...
        for obj in objects:
            if obj is None:
                continue
            cat = obj.category
            if cat in ("NoObject", "Cube", "Score", "Lives"):
                continue
            elif cat == "Player":
                obs["Player"] = {
                    "x": obj.x, "y": obj.y,
            ...
            elif cat in ("Coily", "PurpleBall", "RedBall"):
                enemies.append({
                    "x": obj.x, "y": obj.y,
                    "w": obj.w, "h": obj.h,
                    "dx": obj.dx, "dy": obj.dy,
    ...
    def reset(self, seed=None, options=None):
        ...
        Q*bert has no FIRE-to-start requirement.
        """
