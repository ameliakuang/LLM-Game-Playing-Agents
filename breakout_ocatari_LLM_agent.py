...
class BreakoutOCAtariTracedEnv:
    def __init__(self, 
...
class Policy(Module):
    ...
    def __call__(self, obs):
        predicted_ball_x = self.predict_ball_trajectory(obs)
        target_paddle_pos = self.generate_paddle_target(predicted_ball_x, obs)
    ...
    def select_paddle_action(self, target_paddle_pos, obs):
        ...
        """
        if target_paddle_pos is not None and 'Player' in obs:
...
def test_policy(policy, 
    ...
    env = BreakoutOCAtariTracedEnv(render_mode=None,
                               frameskip=frameskip,
...
def optimize_policy(
    ...
    env = BreakoutOCAtariTracedEnv(env_name=env_name,
    ...
    try:
        ...
        for i in range(n_optimization_steps):
            ...
            if error is None:
                ...
                if mean_rewards >= 300:
                    ...
                    feedback += (f"\nYour score is {mean_rewards} points on average of {num_episodes} games with std dev {std_rewards}. "
                                 f"Try to improve paddle positioning to hit more bricks and lose less lives.")
            ...
            instruction = "In Breakout, you move the bottom paddle left and right to hit the ball to a brick wall at the top of the screen. "
            instruction += "The goal is to keep deflecting the ball to the brick block wall to destroy the bricks upon contact and score points. "
            instruction += "The brick wall consists of six rows of different colored bricks, each worth different points when hit: "
            instruction += "Red bricks (top row) and Orange bricks (second row) are worth 7 points each, "
            instruction += "Yellow bricks (third row) and Green bricks (fourth row) are worth 4 points each, "
            instruction += "Aqua bricks (fifth row) and Blue bricks (bottom row) are worth 1 point each. "
            instruction += "However hitting top rows of bricks would deflect the ball faster and make catching the ball harder. "
